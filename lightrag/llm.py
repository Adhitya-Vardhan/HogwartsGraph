...
async def gemini_complete_if_cache(
    ...
    **kwargs,
    ...
    if api_key:
        os.environ["GOOGLE_API_KEY"] = api_key  # Use the correct environment variable for Gemini
    
    genai.configure(api_key=os.environ.get("GOOGLE_API_KEY"))
    generation_config = genai.GenerationConfig(**kwargs.get('generation_config', {}))


    gemini_model = genai.GenerativeModel(model_name=model, generation_config=generation_config)
    messages = []
    if system_prompt:
        messages.append({"role": "user", "parts": [system_prompt]}) # Gemini uses role user for system
    ...
    logger.debug("Full context:")
    for msg in messages:
      logger.debug(f"{msg['role']}: {msg['parts']}")

    try:
        response = gemini_model.generate_content(contents=messages, stream=kwargs.get('stream', False))
...
async def google_gemini_complete(
    prompt: str,
    ...
    **kwargs
    ...
    for message in history_messages:
        ...
        contents.append({
            "role": role,
            "parts": [{"text": str(message.get("parts", [{}])[0].get("text", ""))}]
        })
    ...
    payload = {
        "contents": contents,
        # Add only Gemini-specific parameters from kwargs
        "generationConfig": kwargs.get("generationConfig", {}),
        "safetySettings": kwargs.get("safetySettings", [])
    }

    # Remove any None values from payload
    payload = {k: v for k, v in payload.items() if v}

    # Convert payload to JSON using custom encoder
    ...
    if keyword_extraction:
        try:
            return locate_json_string_body_from_string(response_text)
        except Exception as e:
...
async def google_embedding(
    text: Union[str, List[str]],
    **kwargs
) -> Union[List[float], List[List[float]]]:
    """
    Fetch embeddings for a single text string or a list of text strings
    ...
    try:
        ...
        # Correct model name for embeddings
        model_name = "models/embedding-001"
        
        # Initialize the GenerativeModel for embeddings
        ...
        embeddings = []
        # Retrieve embeddings using embed_content
        ...
        raise Exception(f"Error calling Google Embedding API: {str(e)}")
